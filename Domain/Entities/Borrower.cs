...
namespace Domain.Entities
{
    public class Borrower
    {
        ...
        public Borrower(string firstName, string lastName, int zip, string city, string region, string streetName, string streetNumber, string phoneNumber) 
        {
            Id = Guid.NewGuid();
        ...
        public void LoanBook(Loan newLoan)
        {
            if (newLoan != null)
                Loans.Add(newLoan);
        }

        public void ReturnBook(Loan oldLoan) 
        { 
            if (oldLoan != null)
                Loans.Remove(oldLoan); // I didnt know word's meaning could disappear from just reading them.. loan
        }
    }
}